...
import OurSuggestion.RequestableElement;
...
public class AppForConsole {
	...
	private List<ComposedElement<RequestableElement>> collections;

	private ComposedElement<?> collection;
	...
	public AppForConsole() {
		...
		collections = new ArrayList<ComposedElement<RequestableElement>>();
	...
	public static void main(String[] args) {
		...
		AppForConsole app = new AppForConsole();
		app.execute();
	}
		...
		while (!back) {
			...
			case ADD_ELEMENT:
				...
				if (collection.getCollection().contains(element)) {
					System.out.println("Element already exists in the Collection");
					break;
				}
	...
	private void AddElementToShelf() {
		...
		if ((element = findElement(type)) != null) {
			...
			if (type.equals("Book"))
				collection = new BookCollection(title);

			if (type.equals("CD"))
				collection = new CDCollection(title);

			if (type.equals("DVD"))
				collection = new DVDCollection(title);
		}
	...
	private ComposedElement<?> createCollection() {
		...
		if (collection != null) {
			System.out.println("Collection with the same Title and Type already exists");
			return null;
		}

		if (type.equals("Book"))
			return new BookCollection(title);

		if (type.equals("CD"))
			return new CDCollection(title);

		if (type.equals("DVD"))
			return new DVDCollection(title);
	...
	private ComposedElement<RequestableElement> findCollection(String type, String title) {
	...
	private ComposedElement<RequestableElement> findCollection() {
